...
<script lang="ts"> 
  ...
  import { useFunctionParameterStore } from '/@/store/modules/functionparameter'; 
  export default defineComponent({
    ...
    setup(_, { emit }) {
      function getThresholdColumns(): BasicColumn[] {
        return [
          ...
          },
          {
            ...
            customRender: ({text, record }) => {
              var children: any = [];
              children.push(h(Select.Option, {  value: 'required' }, () => '不允许为空'))
              children.push(h(Select.Option, { value: 'notrequired' },()=>'允许为空'))
              return h(Select, { style: 'width:120px', value: text, onChange: e => handleSave({ ...record, Constraint: e }) }, () => children);
      ...
      const functionParameterStore = useFunctionParameterStore();
      const striped = ref(true);
      const canResize = ref(false);
      const functionId = ref<number | null>(null);
      const parameterType = ref<string | null>(null);
      const pagination = ref<any>(false);
      const deviceCode = ref<string | null>(null);
      const functionCode = ref<string | null>(null);
      ...
      const jsonData = ref([{
        ...
        FunctionCode: unref(functionCode),
        Code:"",
        Name: "",
        ParameterType: unref(parameterType),
        Constraint: null,
        DataTypeValue: null,
      }]);
      ...
      const [registerModal, { setModalProps, closeModal }] = useModalInner(
        async (data) => {
          ...
          functionId.value = data?.record?.Id;
          functionCode.value = data?.record?.FunctionId;
      ...
      function edit(callback: Function) {
        functionParameterStore
          .editApi({
            key: [functionCode.value, productCode.value, deviceCode.value].join("_"),
            list: jsonData.value
          })
          .then((data) => {
            callback(data);
